...
SQLVarText.prototype.decode = function(data) {
    var ret;
    if ((this.type != SQL_BLOB) || (this.subType > 1)) {
...
function search_blob_column(row, meta) {
    ...
    for (column in meta) {
        if (meta[column] instanceof SQLVarBlob) {
            result.push(asObject[column].toString());                
        }
    }
    return result;
}
...
Connection.prototype.executeStatement = function(transaction, statement, params, callback, custom) {
  // Se agrega un delay de 100ms por grupo de statements para que no se bloquee el motor
  this._pendingExecute++;
  var timeExec = 100 * this._pendingExecute;
  //console.log('prepare execution', timeExec ); 
  setTimeout(() => {	
    //console.log('exec', this._pendingExecute ); 
